...
def get_matchround_fixtures(season_length,match_round,current_round):
    ...
    for matchId in match_ids:
        ...
        home_team = data['general']['home_team']
...
def process_match(match_id, league_id):
    ...
    homeTeam = data['general']['home_team']
...
def get_latest_comp_shotsdata_parallel(season_length=int, league_id=int):
    response = requests.get(f'https://www.fotmob.com/api/leagues?id={league_id}&ccode3=USA_MA')
    data = json.loads(response.content)
    leagues = data['overview']['league_overview_matches']
    match_ids = []
    ...
    # Use multiprocessing to fetch data for each match concurrently
    with multiprocessing.Pool() as pool:
        match_data = list(tqdm(pool.imap(partial(process_match, league_id=league_id), match_ids), total=len(match_ids)))
...
def get_season_matchgoals_data(season, league_id):
    ...
    if 'overview' not in data or 'league_overview_matches' not in data['overview']:
    ...
    for matchId in match_ids:
        ...
        match_details.append({
            ...
            'competitions': data['general']['parentLeagueName'],
            'homeTeamName': data['general']['home_team']['name'],
